...
def get_params():
    ...
    parser.add_argument("--T_p", type=int, default=24)
...
def default_config(data='AIR_BJ'):
    ...
    config.model = edict()
    config.model.T_h = 12  # 默认值，会被 args 覆盖
    config.model.T_p = 24  # 默认值，会被 args 覆盖
    ...
    if config.data.name == 'AIR_N95':
        ...
        config.data.points_per_hour = 1
        config.data.val_start_idx = int(8717 * 0.6)
        config.data.test_start_idx = int(8717 * 0.8)
        # 注意：这里不再重写 config.model，避免覆盖后面统一配置
    ...
    config.epoch = 20  # Number of max training epoch
...
def evals(model, data_loader, epoch, metric, config, clean_data, mode='Test'):
    ...
    if metric.best_metrics['epoch'] == epoch:
        ...
        # 如果是测试模式，额外输出 MAPE
        if mode == 'test':
            message += f" {metric.metrics['mape']:<7.2f}%]"
        else:
            message += "]"
    else:
        message = f" | {metric.metrics['mae']:<7.2f}{metric.metrics['rmse']:<7.2f}"
...
def main(params: dict):
    ...
    # model
    config.model.N = params['N']   # <<< FIX 5: 取消注释，使 --N 参数生效
    config.T_h = config.model.T_h = params['T_h']
    config.T_p = config.model.T_p =  params['T_p']   # <<< FIX 2: 修正为 params['T_p']
    ...
    for sample_strategy, sample_steps in [('ddim_multi', 40)]:
        ...
        # <<< MAPE: 在最终结果中显式打印 MAPE
        message = f'sample_strategy:{sample_strategy}, sample_steps:{sample_steps} Final results in test: MAE={metrics_test.metrics["mae"]:.2f}, RMSE={metrics_test.metrics["rmse"]:.2f}, MAPE={metrics_test.metrics["mape"]:.2f}%\n'
        config.logger.write(message, is_terminal=True)
